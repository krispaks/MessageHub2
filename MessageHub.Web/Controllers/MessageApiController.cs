    ...
    public class MessageApiController : ApiController
    {
        ...
        public async Task<System.Net.Http.HttpResponseMessage> GetFile(string fileId, bool download)
        {
            try
            {
                ...
                {
                    HttpResponseMessage result = new HttpResponseMessage(HttpStatusCode.OK);
                    var stream = this.messageService.RetrieveFiles(fileId);
                    result.Content = stream.Result;
                    result.Content.Headers.ContentType = new MediaTypeHeaderValue("text/plain");
                    return result;
                }
                // if the user is checking if there's attached files to the message
                ...
                {
                    var fileSession = RavenMessageUoW.filesStore.OpenAsyncSession();

                    var query = await fileSession.Query()
                                 .WhereEquals("Message", fileId)
                                 .ToListAsync();
